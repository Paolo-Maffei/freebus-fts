...
{
   /**
    * Read the value of an ASAP (communication object).
    * Response: {@link #GroupValue_Response}.
    */
   GroupValue_Read(0x000, 10, 0, 0),

   /**
    * The response to the {@link #GroupValue_Read} request. The data
    * depends on the type of the communication object.
    *
    ...
    * Two data bytes: the new physical address high/low.
    */
   ...
   /**
    * Read the individual address of a device which is identified by a given
    * KNX serial number. Broadcast.
    * 
    * 6 data bytes: KNX serial number.
    ...
    * 
    * 10 data bytes: 6 bytes KNX serial number, 2 bytes address high/low,
    * 2 bytes reserved.
    */
   IndividualAddressSerialNumber_Response(0x3dd, 10, 10, 10),

   /**
    * Set the serial number of a device  which is identified by a given
    * KNX serial number. Broadcast.
    * 
    * 12 data bytes: 6 bytes KNX serial number, 2 bytes new address high/low,
    * 4 bytes reserved.
    */
    ...
    * 4 data bytes: channel number, read count, sum of A/D converter values high/low.
    */
    ...
    * Also called DeviceDescriptor_InfoReport.
    */
   ...
   /**
    * Modify or delete a key which is associated to a certain access level
    * in the communication parner.
    * 
   ...
   /**
    * Set the domain address of a communication partner.
    * 2+ data bytes: the domain address (high to low byte).
    */
   ...
   /**
    * Response to the {@link #DomainAddress_Read} request.
    * 2+ data bytes: the domain address (high to low byte).
    */
   DomainAddress_Response(0x3e2, 10, 2, 16),

   /**
    * A {@link #DomainAddress_Read} request, but more specific.
    * Read the domain address of a communication partner that is
    * identified within a service. Broadcast.
    */
   DomainAddressSelective_Read(0x3e3, 10, 2, 16),

   /**
    * Read the domain address of a communication partner. The
    * communication partner is identified by its KNX serial number.
    ...
    * 8+ data bytes: 6 bytes KNX serial number, rest is domain address
    * (high to low byte). 
    */
   DomainAddressSerialNumber_Response(0x3ed, 10, -1, -1),

   /**
    * Set the domain address of a communication partner which is
    * identified by its KNX serial number.
    * 8+ data bytes: 6 bytes KNX serial number, rest is domain address
    * (high to low byte).
    */
