...
{
   private MessageCode messageCode = MessageCode.UNKNOWN;
   private int from = 0;
   private int dest = 0;
   private boolean isDestGroup = false;
   private Priority priority = Priority.NORMAL;
   private TransportType transportType = TransportType.DATA;
    ...
    * bit 4: 1=for data telegram and poll-data telegram, 0=for acknowledge telegram
   ...
   {
      if (from<0 || from>65535) throw new IllegalArgumentException("illegal physical address");
   ...
   {
      if (dest<0 || dest>65535) throw new IllegalArgumentException("illegal physical address");
   ...
   {
      if (dest<0 || dest>32767) throw new IllegalArgumentException("illegal group address");
      this.dest = dest & 0xffff;
   ...
   {
      if (isDestGroup) return Integer.toString(dest>>11)+'/'+Integer.toString((dest>>7)&15)+'/'+Integer.toString(dest&127);
      return Integer.toString(dest>>12)+'.'+Integer.toString((dest>>8)&15)+'.'+Integer.toString(dest&255);
   }
   ...
   {
      if (data.length<1 || data.length>16) throw new IllegalArgumentException("Invalid data length");
      this.data = data;
   }
